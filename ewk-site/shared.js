...
/* ── Non-blocking Google Fonts (avoids render-blocking @import chain) ── */
(function(){
  var l = document.createElement('link');
  l.rel = 'stylesheet';
  l.href = 'https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800&family=Poppins:wght@400;500;600;700;800&display=optional';
  l.media = 'print';
  l.onload = function(){ this.media = 'all'; };
  document.head.appendChild(l);
})();

/* ── Auto-inject BreadcrumbList JSON-LD on every non-home page ── */
(function(){
  var path = window.location.pathname;
  var segs = path.replace(/^\/|\/$/g,'').split('/').filter(Boolean);
  if(!segs.length) return;
  var nm = {
    'about':'About Us','services':'Services','blog':'Blog',
    'locations':'Service Areas','contact':'Contact Us','faq':'FAQ',
    'itad':'IT Asset Disposal (ITAD)','pickup':'Free E-Waste Pickup',
    'data-destruction':'Data Destruction','recycling':'E-Waste Recycling',
    'sell-electronics':'Sell Old Electronics','battery-recycling':'Battery Recycling',
    'hard-drive-shredding':'Hard Drive Shredding','e-waste-recycling':'E-Waste Recycling',
    'scrap-price':'Electronic Scrap Prices','pricing':'Pricing',
    'marketplace':'Marketplace','privacy':'Privacy Policy','terms':'Terms & Conditions',
    'data-destruction-services-kochi':'Data Destruction Services Kochi',
    'computer-scrap-buyers-kochi':'Computer Scrap Buyers Kochi',
    'e-waste-collection-kochi':'E-Waste Collection Kochi',
    'e-waste-scrap-prices-kochi':'E-Waste Scrap Prices Kochi',
    'free-e-waste-pickup-kochi':'Free E-Waste Pickup Kochi',
    'electronics-recycling-near-me':'Electronics Recycling Near Me',
    'hard-drive-degaussing-kochi':'Hard Drive Degaussing Kochi',
    'server-recycling-kochi':'Server Recycling Kochi',
    'it-asset-inventory-audit':'IT Asset Inventory Audit'
  };
  var items = [{"@type":"ListItem","position":1,"name":"Home","item":"https://www.ewastekochi.com/"}];
  var base = 'https://www.ewastekochi.com';
  for(var i=0;i<segs.length;i++){
    base += '/'+segs[i];
    var label = nm[segs[i]];
    if(!label){
      label = i===segs.length-1
        ? (document.title||'').replace(/\s*[\|–\-]+\s*Ewaste Kochi.*$/i,'').trim() || segs[i].replace(/-/g,' ').replace(/\b\w/g,function(c){return c.toUpperCase();})
        : segs[i].replace(/-/g,' ').replace(/\b\w/g,function(c){return c.toUpperCase();});
    }
    items.push({"@type":"ListItem","position":i+2,"name":label,"item":base+"/"});
  }
...
const NAV_LINKS = [
  ...
  { label: 'Services', href: '/services/', dropdown: [
    { label: 'IT Asset Disposal (ITAD)', href: '/itad/' },
    { label: 'Data Destruction Services', href: '/data-destruction/' },
    { label: 'E-Waste Recycling', href: '/recycling/' },
    { label: 'Battery Recycling', href: '/battery-recycling/' },
    { label: 'Sell Old Electronics', href: '/sell-electronics/' },
    { label: 'Electronic Scrap Buyers', href: '/scrap-price/' },
    { label: 'Free E-Waste Pickup', href: '/pickup/' },
    { label: 'Marketplace – Buy & Sell', href: '/marketplace/' },
    { label: 'Electronics Recycling Near Me', href: '/services/electronics-recycling-near-me/' },
  ]},
  { label: 'ITAD Solutions', href: '/itad/', dropdown: [
    { label: 'ITAD Services Kochi', href: '/itad/' },
    ...
    { label: 'Server Recycling', href: '/services/server-recycling-kochi/' },
  ...
  { label: 'Scrap Price', href: '/scrap-price/' },
...
const FOOTER_SERVICES = [
  ...
  { label: 'Electronic Scrap Buyers', href: '/scrap-price/' },
  { label: 'Free E-Waste Pickup', href: '/pickup/' },
  { label: 'Free E-Waste Pickup Kochi', href: '/free-e-waste-pickup-kochi/' },
  { label: 'E-Waste Collection Kochi', href: '/e-waste-collection-kochi/' },
...
const FOOTER_AREAS = [
  ...
  { label: 'Thrippunithura', href: '/locations/thrippunithura/' },
  { label: 'Edappally', href: '/locations/edappally/' },
  { label: 'Aluva', href: '/locations/aluva/' },
  { label: 'Kalamassery', href: '/locations/kalamassery/' },
  { label: 'Vyttila', href: '/locations/vyttila/' },
  { label: 'Palarivattom', href: '/locations/palarivattom/' },
  { label: 'Fort Kochi', href: '/locations/fort-kochi/' },
  { label: 'Kadavanthra', href: '/locations/kadavanthra/' },
  { label: 'Kaloor', href: '/locations/kaloor/' },
  { label: 'Infopark Kochi', href: '/locations/infopark-kochi/' },
  { label: 'Smart City Kochi', href: '/locations/smart-city-kochi/' },
  { label: 'Angamaly', href: '/locations/angamaly/' },
];

const FOOTER_HUB_PILLARS = [
  { label: 'IT Asset Disposal Guide', href: '/blog/itad/' },
  { label: 'Data Destruction Guide', href: '/blog/data-destruction/' },
  { label: 'E-Waste Recycling Guide', href: '/blog/recycling/' },
  { label: 'E-Waste Collection Guide', href: '/blog/collection/' },
  { label: 'Enterprise ITAD Solutions', href: '/blog/itad/enterprise/' },
  { label: 'Hospital E-Waste Guide', href: '/blog/collection/hospitals/' },
];
...
function renderHeader(activeLabel = '') {
  ...
  <header>
    <div class="header-inner">
      <a href="/" class="logo">
        <img src="/logo.svg" alt="Ewaste Kochi Logo" width="220" height="60">
      </a>
  ...
  <footer>
    <div class="container">
      <div class="footer-grid">
        ...
        <div>
          <div class="footer-brand-logo">
            <img src="/logo.svg" alt="Ewaste Kochi Logo" width="200" height="55" style="filter: brightness(0) invert(1);">
          </div>
          <p class="footer-desc">Kerala's #1 KSPCB-authorized e-waste recycler and ITAD provider. Committed to 100% data security, NIST-certified destruction, and zero-landfill electronic waste management.</p>
          <div class="footer-auth-badge">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2L3 5v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V5l-9-3zm-2 14.5l-3.5-3.5 1.41-1.41L10 13.67l5.59-5.59L17 9.5 10 16.5z"/></svg>
            KSPCB Authorized: KL/EW/628
          </div>
          <div class="footer-socials">
            <a href="https://www.facebook.com/ewastekochi" class="footer-social" aria-label="Facebook" target="_blank" rel="noopener noreferrer">${SVG.fb}</a>
            <a href="https://www.instagram.com/ewastekochi" class="footer-social" aria-label="Instagram" target="_blank" rel="noopener noreferrer">${SVG.ig}</a>
            <a href="https://www.linkedin.com/company/ewastekochi" class="footer-social" aria-label="LinkedIn" target="_blank" rel="noopener noreferrer">${SVG.li}</a>
            <a href="https://www.youtube.com/@ewastekochi" class="footer-social" aria-label="YouTube" target="_blank" rel="noopener noreferrer">${SVG.yt}</a>
          </div>
        </div>
        <!-- Quick Links -->
        <div>
          ...
          <div class="footer-links">
            <a href="/recycling/">E-Waste Recycling</a>
            <a href="/battery-recycling/">Battery Recycling</a>
            <a href="/scrap-price/">Scrap Price List</a>
            <a href="/sell-electronics/">Sell Electronics</a>
            <a href="/marketplace/">Marketplace – Buy &amp; Sell</a>
            <a href="/faq/">Frequently Asked Questions</a>
            <a href="/contact/">Book Free Pickup</a>
          </div>
          <div class="footer-col-title" style="margin-top:1.5rem">E-Waste Guides</div>
          <div class="footer-links">
            ${FOOTER_HUB_PILLARS.map(p => `<a href="${p.href}">${p.label}</a>`).join('')}
          </div>
        </div>
        <!-- Services -->
        <div>
        ...
        <div style="grid-column: span 1;">
          <div class="footer-col-title">Service Areas Kochi</div>
          <div class="footer-links" style="display: grid; grid-template-columns: 1fr 1fr; gap: 0 1rem;">
            ${FOOTER_AREAS.map(a => `<a href="${a.href}">${a.label}</a>`).join('')}
          </div>
          <a href="/locations/" style="display: block; margin-top: 0.8rem; font-size: 13px; font-weight: 700; color: #00e676; text-decoration: none;">View All 32 Locations →</a>
        </div>
        <!-- Contact -->
        <div>
          ...
          <div class="footer-contact-item">${SVG.pin} ${SITE.address}</div>
      ...
      <div class="footer-bottom">
        <div class="footer-copy">© <script>document.write(new Date().getFullYear())</script> Ewaste Kochi. All Rights Reserved. | CPCB Authorized E-Waste Recycler</div>
...
/* ── Lazy image loaded class (for CSS fade-in) ── */
(function () {
  if ('IntersectionObserver' in window) {
    ...
    document.addEventListener('DOMContentLoaded', function() {
      document.querySelectorAll('img[loading="lazy"]').forEach(function(img) {
        if (img.complete) { img.classList.add('loaded'); } else { imgObs.observe(img); }
      });
    });
  }
})();
...
(function () {
  ...
  btn.style.cssText = 'display:none;position:fixed;bottom:90px;right:24px;width:42px;height:42px;background:var(--green,#1a6b3a);color:#fff;border:none;border-radius:50%;font-size:18px;cursor:pointer;z-index:9990;box-shadow:0 3px 12px rgba(0,0,0,.2);transition:opacity .2s,transform .2s;font-weight:700;line-height:1';
