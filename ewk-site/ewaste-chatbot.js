...
`;
  ...
  /* ── Auto-open / teaser ──────────────────────────────────── */
  // Key shared with wa-funnel.js so both scripts agree on "user dismissed"
  const WA_POPUP_KEY = 'ewk_popup_shown';

  var autoOpenTimer = null;   // ref so teaser dismiss can cancel it
  var teaserDismissed = false;

  window.addEventListener('load', function () {
    if (sessionStorage.getItem(SESS_KEY)) return;

    // Show teaser bubble after 3 s
    setTimeout(function () { if (!isOpen) showTeaser(); }, 3000);

    // Auto-open full chat after 12 s (only if teaser wasn't explicitly dismissed)
    autoOpenTimer = setTimeout(function () {
      if (!isOpen && !teaserDismissed) openChat();
    }, 12000);

    // Desktop exit-intent — fires once; yields to wa-funnel if its popup already showed
    document.addEventListener('mouseleave', function (e) {
      if (e.clientY < 10 && !isOpen && !sessionStorage.getItem(SESS_KEY)) {
        // Claim the exit slot: mark wa-funnel's key so it won't also fire
  ...
  document.getElementById('ewkc-tc').addEventListener('click', function (e) {
    e.stopPropagation();
    // User explicitly dismissed the teaser — cancel the auto-open timer
  ...
  function addBot(text, qrs) {
    ...
    // Strip any HTML tags the AI may have emitted (prompt injection defense).
    // Only markdown [label](url) links are converted — everything else is text.
    ...
    avatar.className = 'ewkc-bav';
    avatar.textContent = 'R';

    var bubble = document.createElement('div');
    bubble.className = 'ewkc-bub ewkc-b';

    // Parse markdown links safely — only allow https:// URLs matching wa.me or ewastekochi.com
    var SAFE_URL = /^https:\/\/(wa\.me|(?:www\.)?ewastekochi\.com)\//;
    var segments = safeText.split(/\[([^\]]{1,80})\]\((https?:\/\/[^)]{1,300})\)/g);
    for (var i = 0; i < segments.length; i++) {
      if (i % 3 === 0) {
        // Plain text segment
        if (segments[i]) bubble.appendChild(document.createTextNode(segments[i]));
      } else if (i % 3 === 1) {
        var linkLabel = segments[i];
        var linkUrl   = segments[i + 1] || '';
        i++; // skip consumed URL capture group
        if (!SAFE_URL.test(linkUrl)) {
          bubble.appendChild(document.createTextNode('[' + linkLabel + ']'));
          continue;
        }
        if (linkUrl.indexOf('wa.me') !== -1) {
          bubble.insertAdjacentHTML('beforeend', waBtn(linkUrl, linkLabel));
        } else {
